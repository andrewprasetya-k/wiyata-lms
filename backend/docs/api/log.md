...
| `auth.mfa.enrolled` | MEDIUM | Phase 11.4 (MFA). Emitted by `MFAService.ConfirmEnrollment` (`POST /me/mfa/confirm`) once the caller's first TOTP code validates against the pending secret and `enabled_at` is set — not at `Enroll` time, since a secret that's generated but never confirmed working isn't a real enrollment yet. Metadata: `user_id`. |
...
| `auth.mfa.verify.failed` | MEDIUM | Phase 11.4 (MFA). Emitted by `AuthService.VerifyMFA` on an invalid TOTP/recovery code — same MEDIUM tier as `auth.password.change.failed`. Metadata: `user_id`, `used_recovery` (bool — whether the attempt was a recovery code rather than a TOTP code). Rate-limited: 5 failed attempts / 15 minutes per user (`middleware.NewMFAVerifyAttemptStore()`, keyed `"mfa_verify:" + userID`), same shape as change-password. |
