...
Pattern: `<domain>.<subject>.<verb_past>`, free-form strings (not a Postgres enum — validated only where relevant in application code). Severity is one of `LOW` / `MEDIUM` / `HIGH` / `CRITICAL` (`domain.LogSeverity*` constants — `CRITICAL` added Phase 10.12, used only by `user.deleted`). 67 actions across 20 service files, grouped by domain below, exactly as implemented (verified against every `logService.Log(...)`/`LogBatch(...)` call site as of Phase 11.4):

### Authentication (`internal/service/auth_service.go`, `mfa_service.go`, `user_service.go`, `email_verification_service.go`, `password_reset_service.go`)

`auth_service.go` also issues/rotates refresh tokens as of Phase 11.3 (the backend half of the refresh-token migration, via the new `edv.refresh_tokens` table, migration `0007`) — see the three new rows below. `mfa_service.go`/`auth_service.go` also implement TOTP-based MFA as of Phase 11.4 (migration `0009`) — see the four `auth.mfa.*` rows below. Phase 11.4.1 added a forced-setup path (`POST /login/mfa-setup/enroll`, `POST /login/mfa-setup/confirm`, both public) for users whose grace period already expired — it reuses the same `MFAService.Enroll`/`ConfirmEnrollment` calls (and therefore the same audit actions) as the existing self-service `POST /me/mfa/enroll`/`confirm`, just driven by `token.UserID` resolved from a `mpt_purpose = "mfa_enroll_required"` preAuthToken instead of a JWT.

**Phase 11.5.1 — IP capture:** `domain.Log.IPAddress` (the column) is now populated for every row logged by `auth_service.go` — `auth.login.success`, `auth.login.failed`, `member.login`, `auth.token.refreshed`, `auth.token.reuse_detected`, `auth.mfa.verified`, `auth.mfa.verify.failed` (both call sites), `auth.mfa.recovery_code.used` — sourced from `RefreshTokenMetadata.IPAddress` (itself `gin.Context.ClientIP()`, read once at the handler boundary via `refreshTokenMetadataFromRequest`), threaded through `domain.ActorContext.IPAddress` into `LogService.buildLogEntry`. Before this phase the column existed but nothing ever wrote to it. `auth.password.reset.requested`/`auth.password.reset.completed` (`password_reset_service.go`) and `auth.logout`/`auth.session.revoked` were deliberately left out — none of those call sites currently receive a request-metadata parameter, so populating them would need a handler→service signature change, not just using a value already in scope. Every log row written before this phase has `ip_address = NULL` permanently — see `docs/api/dashboard.md` §5 for how the Security Dashboard's brute-force detection treats that.
...
| `auth.token.reuse_detected` | HIGH | Phase 11.3, semantics tightened Phase 11.5. Emitted only when a refresh token is presented whose row is revoked **with `revoked_reason = "rotated"`** — i.e. it was already exchanged for a newer token, and this old one is being replayed instead. This is a precise signal: `rft_revoked_reason` (added Phase 11.5) distinguishes this from a token revoked for an expected, benign reason (`"user_revoked"` via the Session Management UI, `"logout"` via `POST /logout`), which now correctly resolves as ordinary `ErrRefreshTokenInvalid` instead — no reuse alarm, no family revoke. A revoked row with a missing/unrecognized reason (e.g. a pre-Phase-11.5 row) still fails closed as reuse, since that's the safer default on this path. Triggers `RefreshTokenRepository.RevokeFamily` (which itself stamps every revoked sibling `revoked_reason = "reuse_detected"`), ending every token in that session family, not just the one presented. Metadata: `user_id`, `family_id`. |
| `auth.logout` | LOW | Phase 11.3. Emitted by `AuthService.Logout` (`POST /logout`). Revokes only the single session the presented refresh token belongs to (`revoked_reason = "logout"`) — siblings in the same family are untouched, unlike the reuse-detection response above. `ActorContext.UserID`/metadata's `user_id` are populated only if the presented token still resolves to a row (best-effort — logout never errors to the caller even for a garbage/already-revoked token, so this log can legitimately have no `user_id` in that case). |
| `auth.session.revoked` | MEDIUM | Phase 11.4 (Session Management UI). Emitted by `AuthService.RevokeSession` (`DELETE /me/sessions/:id`) — a deliberate action taken by the user against their own account, not routine expiry/rotation (hence MEDIUM, not LOW, distinguishing it from `auth.logout`/`auth.token.refreshed`). Revokes with `revoked_reason = "user_revoked"`. `entityType` is `"session"` (the underlying row is `edv.refresh_tokens`, but "session" is the user-facing concept this UI exposes). Metadata: `user_id`, `session_id`, `user_agent`, `ip_address` (whichever device/location was revoked). Actor and target are always the same user — self-service only, no admin-revokes-another-user's-session capability exists. **Resolved Phase 11.5**: revoking the session currently in use used to also trip `auth.token.reuse_detected` (HIGH) as a false positive when that browser's next `/refresh-token` call presented the now-revoked token — `Rotate()` couldn't tell a deliberate revoke apart from genuine replay. Now that `revoked_reason` is set to `"user_revoked"` here, `Rotate()` recognizes it as an ordinary ended session and returns the plain "session expired" outcome instead — no reuse alarm, no family revoke. |
...
| `auth.mfa.enrolled` | MEDIUM | Phase 11.4 (MFA). Emitted by `MFAService.ConfirmEnrollment` once the caller's first TOTP code validates against the pending secret and `enabled_at` is set — not at `Enroll` time, since a secret that's generated but never confirmed working isn't a real enrollment yet. Fires identically whether reached via `POST /me/mfa/confirm` (self-initiated, logged-in user) or `POST /login/mfa-setup/confirm` (Phase 11.4.1 — forced setup after grace period expiry, driven by a preAuthToken instead of a JWT) — `MFAService.ConfirmEnrollment` has no notion of which caller invoked it. Metadata: `user_id`. |
| `auth.mfa.verified` | LOW | Phase 11.4 (MFA). Emitted by `AuthService.VerifyMFA` (`POST /login/mfa-verify`) when the submitted TOTP code is valid — same LOW tier as `auth.login.success`, since this is the routine/expected outcome of an MFA-gated login. Metadata: `user_id`. |
| `auth.mfa.verify.failed` | MEDIUM | Phase 11.4 (MFA). Emitted by `AuthService.VerifyMFA` (`POST /login/mfa-verify`) on an invalid TOTP/recovery code, and by `AuthService.CompleteMFASetup` (`POST /login/mfa-setup/confirm`, Phase 11.4.1) on an invalid enrollment code — same MEDIUM tier as `auth.password.change.failed`. Metadata: `user_id`, plus `used_recovery` (bool) for the `/login/mfa-verify` path or `context: "forced_setup"` for the `/login/mfa-setup/confirm` path, so the two call sites stay distinguishable in the log. Both paths share the same rate limit: 5 failed attempts / 15 minutes per user (`middleware.NewMFAVerifyAttemptStore()`, keyed `"mfa_verify:" + userID`), same shape as change-password. |
| `auth.mfa.recovery_code.used` | HIGH | Phase 11.4 (MFA). Emitted by `AuthService.VerifyMFA` on a **successful** recovery-code verification — deliberately a higher tier than a normal `auth.mfa.verified` (LOW), since a recovery code is only ever used when the user has lost access to their authenticator app/device, which is itself a signal worth an admin's attention. Metadata: `user_id`. |
