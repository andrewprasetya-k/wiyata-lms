...
When Supabase storage is enabled, `fileUrl` is returned as the full absolute public URL generated by the provider, for example `https://<project>.supabase.co/storage/v1/object/public/<bucket>/schools/.../file.pdf`. Clients must use this URL as-is and must not prefix it with the API base URL.

**AI summary note:** Material document summary does not use frontend `fileUrl`. The backend verifies material access, resolves `material -> attachment -> media -> storagePath`, and downloads the object internally from storage. This internal download is separate from the still-deferred protected user-facing media delivery/signed URL work.
