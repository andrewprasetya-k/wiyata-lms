...
- `logs` - append-only audit trail. Extended in Phase 10.4 from 6 to 14 columns (`actor_school_user_id`, `entity_type`, `entity_id`, `scope`, `severity`, `ip_address`, `user_agent`, `correlation_id` added — all nullable). See `backend/docs/api/log.md` for the full architecture, REST/WebSocket contracts, taxonomy, and permission model; §21 below and §26's Phase 10 entry for the summary.
...
- The audit log's database row is always the source of truth; the WebSocket feed (`/api/ws/audit`) is distribution-only, published fire-and-forget strictly after the row already committed, and never required for correctness — a missed/dropped event is invisible to REST. `LogService` (write path) and `LogQueryService` (read path, Phase 10.9) are deliberately separate services so read-surface and real-time work never has to touch the write path. See `backend/docs/api/log.md`.
...
- **Phase 9 — Multi-Role Evaluation and School-Role Combination Validation:** (1) _9.1 (read-only audit)_ concluded invitation and enrollment should stay single-role-per-action: the `enrollments` table's `(enr_scu_id, enr_cls_id)` unique constraint (no `enr_role` column) makes per-class dual-role structurally impossible regardless of invitation design, and teacher invitations never auto-create enrollment anyway, so multi-role invitations wouldn't remove the always-necessary follow-up admin step. This closes the "evaluate multi-role invitation/enrollment" TODO with **no implementation** (Option A). (2) _9.2 (read-only audit)_ traced every remaining single-role-shaped pattern across the frontend and found exactly one real bug: `AdminEnrollments.vue`'s `inferPlacementRole()` silently returns `null` (blocking enrollment UI) for a member holding both `student` and `teacher` — a state only reachable since Batch 2's multi-role editor. (3) _9.3 (implementation)_ resolved that bug **at its source instead of patching `AdminEnrollments.vue`**: after a business-rule decision that `student`+`teacher` and `student`+`admin` should never be valid combinations in the first place (only `admin`+`teacher` is), added `domain.ValidateSchoolRoleCombination` (`backend/internal/domain/role_validation.go`) as the single backend source of truth, called from every mutation path that can change a school membership's role set — `rbacService.SyncUserRoles`/`AssignRoleToUser`, `adminSchoolMemberImportService`'s CSV commit and direct member creation, and `invitationRepository.finalizeInvitationAcceptance` (both accept endpoints) — plus a matching frontend inline-validation guard in `AdminUsers.vue`'s role editor. See §24 for the rule statement and `backend/docs/api/rbac.md` §2 for the full contract.
- **Phase 10 — Audit Log Initiative (10.1–10.10):** Built a full audit-log system from a dormant, zero-caller write path into a live-monitored feature. 10.1–10.3 were read-only audits/design (severity-grouped action inventory, `domain.Log`/`ActorContext`/taxonomy/severity policy design, implementation sequencing) with no code written. 10.4 extended the existing-but-unused `edv.logs` table (migration `0003_extend_logs_for_audit.sql`, 8 new nullable columns), `domain.Log`, `domain.ActorContext`, `LogRepository`, and `LogService.Log`/`LogBatch` — additive only, zero business services touched yet. 10.5–10.8 wired 33 audit actions across RBAC, Member Management (incl. CSV import via `LogBatch`'s parent+child `correlation_id` pattern), Enrollment, Subject Class, School, Platform Bootstrap, Assignment, Grade, and Authentication — see `backend/docs/api/log.md` §4 for the full taxonomy table with severities. 10.9 added the REST read surface: a new, deliberately separate `LogQueryService` (never touching `LogService`, the write path used by every prior sub-phase) plus 4 new endpoints (`GET /logs`, `/logs/:id` super-admin-only unrestricted; `/logs/school/:schoolId/search`, `/logs/school/:schoolId/entries/:id` school-pinned) and the first Audit Viewer frontend page (`frontend/src/pages/common/AuditLogsPage.vue`, shared between `/admin/audit-logs` and `/superadmin/audit-logs`). 10.10 added real-time push: a second `events.AuditBroadcaster` interface (mirroring the existing `events.SidebarBroadcaster` pattern), one additive method on the existing chat `realtime.Hub` (`BroadcastToRoom` — room-wide fanout, since unlike chat/notifications the publisher doesn't know which users are watching), and `GET /api/ws/audit` on its own separate `Hub` instance (so audit connections never share a room namespace with chat). Full architecture, taxonomy, REST/WebSocket contracts, permission matrix, and known limitations: `backend/docs/api/log.md`.
- **Phase 10.11 — Bug Fix and Core Identity Events:** Root-caused and fixed `log_sch_id = NULL` appearing on school-scoped audit rows: standalone `middleware.RequireRole(...)` (no preceding `RequireSchoolMember`) never calls `c.Set("school_id", ...)`, so `buildActorContext` silently produced a nil `SchoolID` — fixed on the 3 affected RBAC routes (`AssignRole`, `RemoveRole`, `UpdateUserRoles`) by prepending `RequireSchoolMember`. Added `member.login` (LOW, school-scoped) — emitted once per login, only when the user has an active school membership, using the membership actually selected as `DefaultContext` (deliberately **not** one row per membership, per the Phase 10.12 audit's Part 2 recommendation against that design). Confirmed no backend logout endpoint exists (logout is client-side JWT clearing only) — `auth.logout` was explicitly not implemented as a result.
- **Phase 10.12 — Complete Business Audit Coverage (Part 1):** Closed the largest remaining coverage gap identified by the prior architecture audit: added audit logging to `MaterialService`, `AssignmentService`'s non-assessment lifecycle (create/update/delete assignment, submission, category-create), `AcademicYearService`, `TermService`, `ClassService`, `SubjectService`, `UserService` CRUD, and `SchoolUserService` enroll/unenroll — 29 new actions (see `backend/docs/api/log.md` §4). Introduced a 4th severity tier, `CRITICAL` (`user.deleted` only) — additive constant, no schema/migration change since severity is app-validated free text. Found and fixed 5 more instances of the same Phase 10.11 middleware bug (`AcademicYear.Create`, `Term.Create`, `Subject.Create`, `SchoolUser.Enroll`, `SchoolUser.Unenroll`). Not implemented: `assignment.category.updated`/`.deleted` (no underlying capability exists in the codebase) and RBAC role-definition CRUD (`CreateRole`/`UpdateRole`, out of this phase's scope). Full taxonomy, metadata deviations, and known limitations: `backend/docs/api/log.md` and `docs/AUDIT_LOGGING.md`.
- **Phase 10.13 — Documentation Sync:** Brought all documentation (this file, `docs/CODEBASE_ANALYSIS.md`, `docs/QUICK_REFERENCE.md`, `backend/docs/API_SUMMARY.md`, `backend/docs/api/log.md`, `backend/schema.md`) up to date with the Phase 10.11–10.12 implementation, and added `docs/AUDIT_LOGGING.md` as the consolidated architecture/roadmap reference. Documentation-only — no backend, frontend, migration, taxonomy, or severity changes.
...
- **An earlier "Setup Progress" section is no longer present in `AdminDashboard.vue`, and its removal is confirmed intentional (resolved, not open debt).** The section, its `setupSteps`/`isSetupComplete` script logic, and its `PhCheckCircle`/`PhCircleDashed` icon usage were added in `fe9d8d5` (2026-07-08) and removed in `6d40d37` (2026-07-21, "feat: modify admin and super admin dashboard") as part of that commit's larger addition of work-queue widgets (classes without teacher, content-less subject-classes, subjects missing assessment-weight config, grading backlog). A new-school onboarding checklist is **not wanted** — the current work-queue widgets cover the operational-health need going forward, and a static one-time setup checklist would be redundant with them. Do not re-add this section.
...
Specialized docs to consult: `README.md`, `README_EN.md`, `TODO.md`, `backend/TODO.md`, `backend/schema.md`, `backend/docs/API_SUMMARY.md`, `backend/docs/api/enrollment.md`, `backend/docs/api/notification.md`, `backend/docs/api/dashboard.md` (student/teacher/admin/super-admin dashboard contracts, including Phase 7 fields — see §25), `backend/docs/api/invitation.md` (public invitation accept flow, including the Phase 8 existing-user path — see §14), `backend/docs/api/school_member_invitations.md` (admin-facing invitation create/list/revoke), `backend/docs/api/rbac.md` (role/user-role management, including the Phase 9.3 school-role combination rule — see §24), `backend/docs/api/school_member_import.md` (direct member creation/CSV import, same combination rule applies), `backend/docs/api/log.md` (audit log: REST + WebSocket contracts, full action taxonomy with severities, permission matrix — Phase 10.1–10.12, see §26), `docs/AUDIT_LOGGING.md` (consolidated audit log architecture, write/read/realtime flow, full taxonomy, and roadmap — the narrative companion to `log.md`'s contract detail). Older analysis/reference docs in `docs/ANALYSIS_INDEX.md`, `docs/CODEBASE_ANALYSIS.md`, `docs/QUICK_REFERENCE.md`, and `docs/PRODUCT_SCOPE.md` must be verified against current code before relying on details. (`backend/docs/api/school_registration_requests.md`, which documented the removed flow, has been deleted — see section 13 for the current self-service Create School flow.)
