...
async function removeFile(index: number) {
  ...
  try {
    ...
    const currentIndex = files.value.findIndex(
      (item, itemIndex) => fileKey(item, itemIndex) === key,
    );
    if (currentIndex >= 0) {
...
<template>
  <div class="media-uploader">
    <label
      class="flex h-32 w-full cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface-subtle transition hover:border-brand-line hover:bg-surface focus-within:border-brand focus-within:bg-surface focus-within:ring-2 focus-within:ring-brand/15"
    >
      <div
        class="flex flex-col items-center justify-center px-4 py-5 text-center"
      >
        ...
        <p class="text-xs text-muted">
          PDF, video, atau dokumen (maks. {{ maxSizeMb }}MB)
        </p>
      </div>
    ...
    <div v-if="files.length > 0" class="mt-4 space-y-2">
      <div
        ...
        :key="index"
        class="flex max-w-full items-center justify-between gap-3 overflow-hidden rounded-xl border border-border bg-surface p-3"
      >
        <div class="flex min-w-0 flex-1 items-center gap-3 overflow-hidden">
          <div
            class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#f3f0ea]"
          >
        ...
        <div class="flex shrink-0 items-center gap-3">
          <span
            ...
            class="max-w-28 animate-pulse truncate text-xs text-brand"
            >Mengunggah...</span
          >
          <span
            v-else-if="isFileRemoving(file, index)"
            class="max-w-28 animate-pulse truncate text-xs text-brand"
            >Menghapus...</span
          >
          <span
            v-else-if="file.status === 'error'"
            class="max-w-36 truncate text-xs text-danger"
            >{{ file.errorMessage }}</span
          >
